...
async def getGameInfo(appid: int):
    ...
    try:
        ...
        if app_data and app_data.get('success'):
            ...
            if details:
                ...
                try:
                    ...
                    log_error("steamFinder.getGameInfo", f"处理价格异常: {e}")
                    initial = 1
                    final = 1
                    currency = None
                    print(errors[-1])
        ...
        log_error("steamFinder.getGameInfo", f"请求API异常: {e}")
        print(errors[-1])
    except json.JSONDecodeError as e:
        errors.append(f"解析API响应JSON时发生错误: {e}")
        log_error("steamFinder.getGameInfo", f"JSON解析异常: {e}")
        print(errors[-1])
    except Exception as e:
        errors.append(f"处理API响应时发生未知错误: {e}")
        log_error("steamFinder.getGameInfo", f"未知异常: {e}")
        print(errors[-1])
...
async def take_screenshot(appid: str):
    ...
    try:
        ...
        log_error("steamFinder.take_screenshot", f"Playwright 初始化异常: {e}")
        return None
    print("new_browser")
    if not page:
        log_error("steamFinder.take_screenshot", "Playwright初始化失败")
        return None
    try:
        await page.goto(url)
    except Exception as e:
        log_error("steamFinder.take_screenshot", f"页面跳转失败: {e}")
        return None
    print("page_goto")
    try:
        title = await page.title()
        if title == "Welcome to Steam":
            log_error("steamFinder.take_screenshot", f"AppID {appid} 不存在（跳转至Welcome to Steam）")
            return None
        ...
        log_error("steamFinder.take_screenshot", f"页面处理异常: {e}")
        return None
    ...
    try:
        ...
        log_error("steamFinder.take_screenshot", "截图超时，30秒内元素未出现")
        screenshot_bytes = None
    except PlaywrightError as e:
        msg = e.message if hasattr(e, 'message') else str(e)
        log_error("steamFinder.take_screenshot", f"页面打开失败: {msg}")
        screenshot_bytes = None
