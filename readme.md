...
> **更新流程：** 检测到更新后，机器人先把更新状态写入项目根目录的 `.lock` 标记文件，再发送「🔄 更新成功」提示。提示为尽力送达（best-effort）：若 `nb run` 的自动重载恰好在拉取瞬间关闭了连接导致提示发送失败，不会报错——重启完成后会依据 `.lock` 自动补发「✅ 更新完成 + 本次提交列表」，保证用户一定能收到结果。默认**不主动退出进程** —— `nb run` 自带自动重载（reload），检测到代码文件变化后会优雅重启并加载新代码。
>
